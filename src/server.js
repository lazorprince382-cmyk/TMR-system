...
app.post('/api/learners',auth,asyncRoute(async(req,res)=>{const b=req.body,client=await db.pool.connect();try{await client.query('BEGIN');const {rows}=await client.query('INSERT INTO learners(admission_no,name,email,phone,photo_url,address,health_status,guardian_name,guardian_phone) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',[b.admission_no,b.name,b.email||null,b.phone||null,b.photo_url||null,b.address||null,b.health_status||null,b.guardian_name||null,b.guardian_phone||null]);let programId=b.program_id;if(req.user.role!=='admin'){const p=await client.query('SELECT id FROM programs WHERE name=$1 AND active',[trainerProgramName(req.user.role)]);programId=p.rows[0]?.id}if(programId)await client.query('INSERT INTO enrollments(learner_id,program_id) VALUES($1,$2) ON CONFLICT(learner_id,program_id) DO UPDATE SET active=true',[rows[0].id,programId]);await client.query('COMMIT');res.status(201).json(rows[0]);}catch(e){await client.query('ROLLBACK');throw e}finally{client.release()}}));
app.put('/api/learners/:id',auth,admin,asyncRoute(async(req,res)=>{const b=req.body;const {rows}=await db.query(`UPDATE learners SET name=$1,admission_no=$2,email=$3,phone=$4,photo_url=$5,address=$6,health_status=$7,guardian_name=$8,guardian_phone=$9 WHERE id=$10 RETURNING *`,[b.name,b.admission_no,b.email||null,b.phone||null,b.photo_url||null,b.address||null,b.health_status||null,b.guardian_name||null,b.guardian_phone||null,req.params.id]);if(!rows[0])return res.status(404).json({error:'Learner not found.'});if(b.program_id)await db.query('INSERT INTO enrollments(learner_id,program_id) VALUES($1,$2) ON CONFLICT(learner_id,program_id) DO UPDATE SET active=true',[req.params.id,b.program_id]);res.json(rows[0]);}));
